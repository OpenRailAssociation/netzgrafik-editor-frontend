...
export class NodeService implements OnDestroy {
  ...
  changeNodePosition(
    ...
    this.changeNodePositionWithoutUpdate(nodeId, newPositionX, newPositionY, dragEnd);
    if (enforceUpdate) {
      this.nodesUpdated();
      this.connectionsUpdated();
      this.transitionsUpdated();
  ...
  addTransition(node: Node, trainrun: Trainrun, isNonStop?: boolean) {
  ...
  addTransitionToNodeForTrainrunSections(
    ...
    node.addTransition(port1, port2, trainrunSection1.getTrainrun(), forceStop ? false : undefined);
  }
  ...
  reorderPortsOnNodesForTrainrun(trainrun: Trainrun) {
    this.nodesStore.nodes.forEach((node) => {
