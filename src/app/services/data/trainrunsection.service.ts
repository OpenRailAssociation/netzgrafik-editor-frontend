...
export class TrainrunSectionService implements OnDestroy {
  ...
  addIntermediateStopOnTrainrunSection(trainrunSection: TrainrunSection) {
    ...
    return this.replaceIntermediateStopWithNode(
  ...
  removeIntermediateStopOnTrainrunSection(initialTrainrunSection: TrainrunSection) {
    ...
    while (backwardIterator.hasNext() && !nodeRemoved) {
      ...
      if (
        !node.isNonStopNode() &&
        node.getBetriebspunktName() === $localize`:@@app.models.node.shortNameDefault:NEW` &&
        node.getFullName() === $localize`:@@app.models.node.full-name-default:New node`
      ) {
  ...
  public getNumberOfCollapsedStops(section: TrainrunSection) {
