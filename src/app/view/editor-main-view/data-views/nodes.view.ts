...
export class NodesView {
  ...
  constructor(private editorView: EditorView) {
    this.draggable = d3
      ...
      .on("start", (event: MouseEvent, n: NodeViewObject) => this.onNodeDragStart(event, n.node))
      .on("drag", (event: MouseEvent, n: NodeViewObject) => this.onNodeDragged(event, n.node))
      .on("end", (event: MouseEvent, n: NodeViewObject) => this.onNodeDragEnd(event, n.node));
  ...
  onNodeDragStart(event: MouseEvent, node: Node) {
    D3Utils.enableFastRenderingUpdate();
    const domObj = event.currentTarget as SVGElement;
    ...
    this.dragPreviousMousePosition =
      this.editorView.svgMouseController.getCurrentMousePosition(event);
    this.editorView.pauseUndoRecording();
  }

  onNodeDragged(event: MouseEvent, node: Node) {
    this.editorView.enableElementDragging();
    this.doDrag(event, node.getId());
    this.editorView.disableElementDragging();
  }

  onNodeDragEnd(event: MouseEvent, node: Node) {
  ...
  onNodeDockableMouseover(event: MouseEvent, node: Node) {
    this.hoverNodeDockable(event, node);
  }

  onNodeDockableMouseout(event: MouseEvent, node: Node) {
    this.unhoverNodeDockable(event, node);
  }
  ...
  onNodeLabelAreaMouseover(event: MouseEvent, node: Node) {
    this.hoverNode(event, node);
  ...
  onNodeMouseout(event: MouseEvent, node: Node) {
  ...
  hoverNode(event: MouseEvent, node: Node) {
    const domObj = event.currentTarget as SVGElement;
    if (domObj !== null) {
  ...
  unhoverNode(event: MouseEvent, node: Node) {
    const domObj = event.currentTarget as SVGElement;
    if (domObj !== null) {
  ...
  hoverNodeDockable(event: MouseEvent, node: Node) {
  ...
  unhoverNodeDockable(event: MouseEvent, node: Node) {
    d3.selectAll(StaticDomTags.NODE_DOCKABLE_DOM_REF)
      .filter((n: NodeViewObject) => n.node.getId() === node.getId())
      .classed(StaticDomTags.TAG_HOVER, false);
    this.unhoverNode(event, node);
  ...
  private doDrag(event: MouseEvent, nodeId: number, round = 1, dragEnd = false) {
    if (this.editorView.editorMode === EditorMode.Analytics) {
      return;
    }
    ...
    const currentMousePosition = this.editorView.svgMouseController.getCurrentMousePosition(event);
            ...
            if (obj.attr(StaticDomTags.EDGE_NODE_ID) === "" + node.getId()) {
              if (obj.attr(StaticDomTags.EDGE_ID) !== "" + ts.getId()) {
                if (obj.attr(StaticDomTags.EDGE_LINE_LINE_ID) !== "" + ts.getTrainrunId()) {
                  ...
                  const portTo = node
                    .getPorts()
                    .find((p) => "" + p.getTrainrunSectionId() === trID);
