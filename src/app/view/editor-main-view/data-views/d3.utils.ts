...
export class D3Utils {
  ...
  static doGrayoutTrainrunSectionPin(trainrunSection: TrainrunSectionViewObject, node: Node) {
  ...
  static doGrayout(tsvo: TrainrunSectionViewObject, grayoutEdgeLinePinNode: Node = undefined) {
    d3.selectAll(StaticDomTags.EDGE_LINE_ARROW_DOM_REF)
      .filter((d: TrainrunSectionViewObject) => {
        ...
        return d.firstSection.getId() === tsvo.firstSection.getId() || d.firstSection.getId() === tsvo.lastSection.getId();
      })
      .classed(StaticDomTags.TAG_SELECTED, false)
      .classed(StaticDomTags.TAG_HOVER, false)
      .classed(StaticDomTags.TAG_WARNING, false)
    ...
    if (grayoutEdgeLinePinNode !== undefined) {
      d3.selectAll(StaticDomTags.EDGE_LINE_PIN_DOM_REF + "." + StaticDomTags.EDGE_IS_SOURCE)
        .filter((d: TrainrunSectionViewObject) => {
          ...
          return (
            (d.firstSection.getId() === tsvo.firstSection.getId() || d.firstSection.getId() === tsvo.lastSection.getId()) &&
