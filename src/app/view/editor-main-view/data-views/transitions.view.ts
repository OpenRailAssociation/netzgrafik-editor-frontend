...
export class TransitionsView {
  ...
  onTransitionButtonOut(trainrun: Trainrun, domObj: any, transition: Transition) {
    ...
    const allSections = this.trainrunSectionService.getAllTrainrunSectionsForTrainrun(trainrun.getId());
    ...
    const tsvo1 = new TrainrunSectionViewObject(this.editorView,
      groups.find((group) => group.some((trs) => trs.getId() === trainrunSection1.getId())),
    );
    const tsvo2 = new TrainrunSectionViewObject(this.editorView,
