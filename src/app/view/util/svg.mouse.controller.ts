...
export class SVGMouseController {
  ...
  private svgDrawingContext: d3.Selection<HTMLElement, undefined, HTMLElement, undefined>;
  ...
  private onGraphContainerMousedown(event: MouseEvent) {
    ...
    if (
      event.shiftKey ||
      (event.buttons === 2 && this.lastMouseEventTimeStamp === undefined)
    ) {
  ...
  private onMouseDownZoom(event: MouseEvent) {
    const d = d3.select(event.srcElement);
